...
__all__ = ['CommonSym', 'Discard', 'MemCfg', 'MMap', 'XRef', 'ImpObj',
           'ImpRsnObj', 'ImpRsnSym', 'ArImp', 'LinkMap',
           #'parse'
          ]
...
def parse_mmap(ls: Sequence[str]) -> Sequence[MMap]:
    ...
    for l in ls:
        ...
        #print(repr(l), w[0])
        if not w[0].startswith("0x"):
            raise Exception("welp, bad symbol addr %s"%w[0])
...
def parse(s: str) -> LinkMap:
    if isinstance(s, Path) or os.path.exists(s):
        with open(s, 'r') as f:
            return parse_impl(f.read())
    elif isinstance(s, (bytes, bytearray)):
        return parse_impl(s.decode('utf-8'))
    elif isinstance(s, str):
        return parse_impl(s)
    else:
        rrr = f.read()
        assert isinstance(rrr, (bytes, bytearray, str)), rrr
        return parse(rrr)
